...
unsigned int DilationFilter::nPorts(PortType portType) const
{
...
NodeDataType DilationFilter::dataType(PortType portType, PortIndex portIndex) const
{
    return PixmapData().type();
}

std::shared_ptr<NodeData> DilationFilter::outData(PortIndex port)
{
    return _nodeData;
}

void DilationFilter::setInData(std::shared_ptr<NodeData> nodeData, PortIndex port)
{
//    _nodeData = nodeData;
...
bool DilationFilter::eventFilter(QObject *object, QEvent *event)
{
    if (object == _label)
    {
      int w = _label->width();
